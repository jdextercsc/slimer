...
options:
    ...
    source:
        required: false
        description: the IP address to filter for source of packets in form of
        CIDR notation XX.XX.XX.XX/YY
    dest:
        required: false
        description: the IP address to filter for destination of packets in
        form of CIDR notation XX.XX.XX.XX/YY
    comment:
        required: false
...
def main():
    ...
    if 'source' in module.params and bool(module.params['source']):
        cmd_string += " -s %(source)s" % module.params
    if 'dest' in module.params and bool(module.params['dest']):
        cmd_string += " -d %(dest)s" % module.params
    if 'comment' in module.params and bool(module.params['comment']):
